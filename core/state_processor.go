...
func (p *StateProcessor) Process(block *types.WorkObject, batch ethdb.Batch) (types.Receipts, []*types.Transaction, []*types.Log, *state.StateDB, uint64, uint64, uint64, *multiset.MultiSet, error) {
	...
	for i, tx := range block.Transactions() {
		...
		if tx.Type() == types.QiTxType {
			...
			if block.Coinbase().IsInQiLedgerScope() {
				...
				if primeTerminus == nil {
					return nil, nil, nil, nil, 0, 0, 0, nil, fmt.Errorf("could not find prime terminus header %032x", header.PrimeTerminus())
				}
		...
		if tx.Type() == types.ExternalTxType {
			...
			if types.IsCoinBaseTx(tx) {
				...
				if err != nil {
					return nil, nil, nil, nil, 0, 0, 0, nil, errors.New("coinbase address is not in the chain scope")
				}
	...
	multiSet, utxoSetSize := p.engine.Finalize(p.hc, batch, block, statedb, false, utxosCreatedDeleted.UtxosCreatedHashes, utxosCreatedDeleted.UtxosDeletedHashes)
	...
	utxosCreatedDeleted = nil // Release memory
