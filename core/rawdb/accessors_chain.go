...
func WriteNewLockups(db ethdb.KeyValueWriter, readDb ethdb.Reader, blockHash common.Hash, newLocks map[common.InternalAddress]*big.Int, newUnlocks []common.Unlock) {
	...
	for addr, amount := range newLocks {
		db.Logger().Debugf("Address %s has new lockup %s\n", addr.String(), amount.String())
	...
	for _, unlock := range newUnlocks {
		db.Logger().Debugf("Address %s has new unlock %s\n", unlock.Addr.String(), unlock.Amt.String())
	...
	for addr, delta := range deltas {
		...
		if value.Sign() < 0 {
			...
			db.Logger().Debugf("Address %s setting lockup %s delta %s\n", addr.String(), value.String(), delta.String())
...
func UndoNewLockupsForBlock(db ethdb.KeyValueWriter, readDb ethdb.Reader, blockHash common.Hash, logger *log.Logger) {
	logger.Debugf("Undoing lockups for block %s\n", blockHash)
	...
	for _, delta := range protoDeltas.KeysAndValues {
		...
		if value.Sign() < 0 {
			...
			db.Logger().Debugf("Address %s setting lockup %s amount %s in reorg\n", addr.String(), value.String(), amount.String())
			if err := db.Put(addressLockupsKey(addr), value.Bytes()); err != nil {
				db.Logger().WithField("err", err).Error("Failed to store new lockups")
			}
		}
