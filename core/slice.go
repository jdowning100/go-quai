...
func (sl *Slice) Append(header *types.WorkObject, domPendingHeader *types.WorkObject, domTerminus common.Hash, domOrigin bool, newInboundEtxs types.Transactions) (types.Transactions, bool, bool, error) {
	...
	sl.logger.Infof("NewInboundEtxsFromDom: len=%d", len(newInboundEtxs))
	...
	if !domOrigin && nodeCtx != common.ZONE_CTX {
		...
		if exists && cachedInboundEtxs != nil {
			sl.logger.Infof("Found the inbound etxs in cache len=%d", len(cachedInboundEtxs))
			...
			sl.logger.Infof("CollectNewlyConfirmedEtxs: len=%d hash %s inbound etxs", len(newInboundEtxs), block.Hash())
	...
	sl.logger.Infof("InboundEtxsAfterSubAppend: len=%d", len(newInboundEtxs))
...
func (sl *Slice) CollectNewlyConfirmedEtxs(block *types.WorkObject) (types.Transactions, types.Transactions, error) {
	...
	if blockLocation.Equal(common.Location{0, 1}) {
		sl.logger.Infof("FilterToSlice len=%d subrollup=%d ETXs destined to this slice: %s block number %d block hash %s", len(newInboundEtxs), len(subRollup), blockLocation.Name(), block.NumberU64(nodeCtx), block.Hash().String())
	}
	...
	if blockLocation.Equal(common.Location{0, 1}) {
		sl.logger.Infof("FilterConfirmationCtx len=%d subrollup=%d ETXs destined to this slice: %s block number %d block hash %s", len(newlyConfirmedEtxs), len(subRollup), blockLocation.Name(), block.NumberU64(nodeCtx), block.Hash().String())
	}
	for {
		...
		if blockLocation.Equal(common.Location{0, 1}) {
			sl.logger.Infof("FilterToSlice len=%d subrollup=%d ETXs destined to this slice: %s block number %d block hash %s", len(ancInboundEtxs), len(subRollup), blockLocation.Name(), parent.NumberU64(nodeCtx), parent.Hash().String())
		}
		...
		block = parent
		if blockLocation.Equal(common.Location{0, 1}) {
			sl.logger.Infof("CollectAncestorEtxs len=%d ETXs", len(newlyConfirmedEtxs))
		}
	}
...
func (sl *Slice) GetPendingEtxsRollupFromSub(hash common.Hash, location common.Location) (types.PendingEtxsRollup, error) {
	...
	if nodeCtx == common.PRIME_CTX {
		...
		if block != nil {
			...
			sl.logger.Infof("Collected sub rollup from sub len=%d", len(subRollup))
...
func (sl *Slice) AddPendingEtxsRollup(pEtxsRollup types.PendingEtxsRollup) error {
	...
	if !sl.hc.pendingEtxsRollup.Contains(pEtxsRollup.Header.Hash()) {
		...
		sl.logger.WithField("len", len(pEtxsRollup.EtxsRollup)).Info("PendingEtxRollup already exists")
	}
	return nil
}
